...
function onDeviceReady() {
    //During testing, Let me know PhoneGap actually initialized
    alert("onDeviceReady");
}

function getContact(contact) {
    //Build a string representing the file name we're going after
    var contactFile = "contacts/" + contact + ".js";
    //Get the contact information from the file system then call the callback 
    //function to parse the data
    $.getJSON(contactFile, showContact);
}

function showContact(contactData) {
    //Populate the Contact Details page with information about this contact
    var dt = "";
    //Store the contact data in our global variable so the
    // AddContact function will have something to work with
    contactInfo = contactData;
    //First set the header content for the page to match the contact's full name
    $('#contactName').text(contactData.FullName);
    //Then populate the body of the content area with detailed fields from the data source
    ...
    $.mobile.changePage("#contactDetail", { transition: "slide" });
}
...
function onContactSaveError(e) {
    var msgText;
    //Now build a message string based upon the error returned by the API
    switch (e.code) {
...
function searchContacts() {
    ...
    $('#searchContacts').html('<p>Search Results</p>');
    ...
    if (len > 0) {
        ...
        for (i = 0, len; i < len; i += 1) {
            ...
            if (contacts[i].displayName == null) {
                theList += '<li><a onclick="showSearchContact(' + i + ');">' + contacts[i].name.familyName + ", " + contacts[i].name.givenName + '</a></li>';
            } else {
                theList += '<li><a onclick="showSearchContact(' + i + ');">' + contacts[i].displayName + '</a></li>';
            }
        }
        theList += '</ul>';
        //$('#searchContacts').replaceWith(theList);
        $('#searchContacts').html(theList);
        //$('#searchContacts').text(theList);
        //Then switch to the Contact Details page
        $.mobile.changePage("#contactList",  { transition: "slide" });
    } else {
...
function showSearchContact(index) {
    var len, i;
    //Populate the Contact Details page with information about this contact
    ...
    //First set the header content for the page to match the contact's full name
    //Unfortunately iOS doesn't use displayName, so this had to be rewritten

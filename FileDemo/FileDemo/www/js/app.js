...
function processDir(fileSystemType) {
    navigator.notification.alert("processDir: " + fileSystemType);
    //Get a handle to the local file system (allocate 1 Mb for storage)
    window.requestFileSystem(fileSystemType, 1024 * 1024, onGetFileSystemSuccess, onFileError);
}

function onGetFileSystemSuccess(fs) {
    navigator.notification.alert("onGetFileSystemSuccess: " + fs.name);
    //Save the file system object so we can access it later
    theFileSystem = fs;
    //Create a directory reader we'll use to list the files in the directory
...
function onDirReaderSuccess(dirEntries) {
    navigator.notification.alert("onDirReaderSuccess (" + dirEntries.length + ")");
    //Empty the previous dir entries
    $('#dirEntries').empty();
    ...
    $.mobile.changePage("#dirList", { transition: "slide" });
}

function processEntry(entryIndex) {
    //clear out the writeInfo div in case we go back to the list page
    $('#writeInfo').empty();
    ...
    $('#fileInfo').html(fi);
    //Display the directory entries page
    $.mobile.changePage("#fileDetails", { transition: "slide" });
...
function writeFile() {
    ...
    navigator.notification.alert("writeFile: " + theFileName);
...
function onGetFileSuccess(theFile) {
    navigator.notification.alert("onGetFileSuccess: " + theFile.name);
    theFile.createWriter(onCreateWriterSuccess, onFileError);
}

function onCreateWriterSuccess(writer) {
    navigator.notification.alert("onCreateWriterSuccess");
...
function onRemoveFileSuccess(entry) {
    navigator.notification.alert("Successfully removed " + entry.name);
}

function viewFile() {
    $('#viewFileName').html('<h1>' + theEntry.name + '</h1>');
    //Display the directory entries page
    $.mobile.changePage("#viewFile", { transition: "slide" });
